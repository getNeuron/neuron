...
public:
    ...
    int64_t GetNeuronBalance() const;
    ...
    int64_t GetNeuronDebit(const CTxIn& txin) const;
    int64_t GetNeuronCredit(const CTxOut& txout) const;
    ...
    bool GetCredit(const CTransaction& tx, int64_t& nNRN, int64_t& nNeuron) const
    {
        nNRN = 0;
        nNeuron = 0;
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txout.IsAnonOutput())
            {
                nNeuron += GetNeuronCredit(txout);
            } else
                nNRN += GetCredit(txout);
            if (!MoneyRange(nNRN)
                || !MoneyRange(nNeuron))
                throw std::runtime_error("CWallet::GetCredit() : value out of range");
        }
...
public:
    ...
    mutable bool fAvailableNeuronCreditCached;
    mutable bool fCreditSplitCached;
    mutable int64_t nDebitCached;
    mutable int64_t nCreditCached;
    mutable int64_t nAvailableCreditCached;
    mutable int64_t nChangeCached;
    mutable int64_t nAvailableNeuronCreditCached;
    
    mutable int64_t nCredNRNCached;
    mutable int64_t nCredNeuronCached;
    ...
    void Init(const CWallet* pwalletIn)
    {
        ...
        fAvailableNeuronCreditCached = false;
        ...
        nAvailableNeuronCreditCached = 0;
        nChangeCached = 0;
        nOrderPos = -1;
    }
    ...
    bool GetCredit(int64_t& nCredNRN, int64_t& nCredNeuron, bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
        {
            nCredNRN = nCredNeuron = 0;
            return true;
        }
        ...
        if (!fUseCache || !fCreditSplitCached)
        {
            nCredNRNCached = 0;
            nCredNeuronCached = 0;
            pwallet->GetCredit(*this, nCredNRNCached, nCredNeuronCached);
            fCreditSplitCached = true;
        };
        
        nCredNRN = nCredNRNCached;
        nCredNeuron = nCredNeuronCached;
        return true;
    }
    ...
    int64_t GetAvailableNeuronCredit(bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
            return 0;
        
        if (fUseCache && fAvailableNeuronCreditCached)
            return nAvailableNeuronCreditCached;
        ...
        for (unsigned int i = 0; i < vout.size(); i++)
        {
            if (!IsSpent(i))
            {
                ...
                if (!MoneyRange(nCredit))
                    throw std::runtime_error("CWalletTx::GetAvailableNeuronCredit() : value out of range");
            };
        };
        
        nAvailableNeuronCreditCached = nCredit;
        fAvailableNeuronCreditCached = true;
        return nCredit;
    };
