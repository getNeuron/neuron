...
NeuronGUI::NeuronGUI(QWidget *parent):
    QMainWindow(parent),
    bridge(new NeuronBridge(this)),
    clientModel(0),
    walletModel(0),
    messageModel(0),
    ...
    nWeight(0)
{
    ...
    resize(1280, 720);
    setWindowTitle(tr("Neuron") + " - " + tr("Client"));
#ifndef Q_OS_MAC
    qApp->setWindowIcon(QIcon(":icons/neuron"));
    setWindowIcon(QIcon(":icons/neuron"));
#else
...
#ifdef Q_OS_WIN
    QFile html("C:/neuron/index.html");
#else
    QFile html("/opt/neuron/index.html");
#endif
...
void NeuronGUI::pageLoaded(bool ok)
{
...
void NeuronGUI::addJavascriptObjects()
{
    documentFrame->addToJavaScriptWindowObject("bridge", bridge);

}

void NeuronGUI::urlClicked(const QUrl & link)
{
...
void NeuronGUI::createActions()
{
    ...
    aboutAction = new QAction(QIcon(":/icons/neuron"), tr("&About Neuron"), this);
    aboutAction->setToolTip(tr("Show information about Neuron"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(QIcon(":/trolltech/qmessagebox/images/qtlogo-64.png"), tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(QIcon(":/icons/options"), tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for Neuron"));
    optionsAction->setMenuRole(QAction::PreferencesRole);
    toggleHideAction = new QAction(QIcon(":/icons/neuron"), tr("&Show / Hide"), this);
...
void NeuronGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if (clientModel)
    {
        ...
        if (mode != NT_FULL)
        {
            ...
            setWindowTitle(tr("Neuron") + " - " + tr("Wallet") + ", " + sMode);
        };
...
#ifndef Q_OS_MAC
            qApp->setWindowIcon(QIcon(":icons/neuron_testnet"));
            setWindowIcon(QIcon(":icons/neuron_testnet"));
#else
            MacDockIconHandler::instance()->setIcon(QIcon(":icons/neuron_testnet"));
#endif
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("Neuron client") + QString(" ") + tr("[testnet]"));
                trayIcon->setIcon(QIcon(":/icons/neuron_testnet"));
...
void NeuronGUI::setWalletModel(WalletModel *walletModel)
{
...
void NeuronGUI::setMessageModel(MessageModel *messageModel)
{
...
void NeuronGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("Neuron client"));
    trayIcon->setIcon(QIcon(":/icons/neuron"));
...
#ifndef Q_OS_MAC
void NeuronGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void NeuronGUI::setNumConnections(int count)
{
    ...
    connectionsIcon.setAttribute("data-title", tr("%n active connection(s) to Neuron network", "", count));
}

void NeuronGUI::setNumBlocks(int count, int nTotalBlocks)
{
...
void NeuronGUI::error(const QString &title, const QString &message, bool modal)
{
    // Report errors from network/worker thread
    if(modal)
    {
...
void NeuronGUI::changeEvent(QEvent *e)
{
...
void NeuronGUI::closeEvent(QCloseEvent *event)
{
    if(clientModel)
    {
#ifndef Q_OS_MAC // Ignored on Mac
...
void NeuronGUI::askFee(qint64 nFeeRequired, bool *payFee)
{
    QString strMessage =
        tr("This transaction is over the size limit.  You can still send it for a fee of %1, "
          "which goes to the nodes that process your transaction and helps to support the network.  "
          "Do you want to pay the fee?").arg(
                BitcoinUnits::formatWithUnit(BitcoinUnits::NRN, nFeeRequired));
...
void NeuronGUI::incomingTransaction(const QModelIndex & parent, int start, int end)
{
...
void NeuronGUI::incomingMessage(const QModelIndex & parent, int start, int end)
{
    if(!messageModel)
        return;
...
void NeuronGUI::optionsClicked()
{
    bridge->triggerElement("#navitems a[href=#options]", "click");
    showNormalIfMinimized();
}

void NeuronGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void NeuronGUI::dragMoveEvent(QDragMoveEvent *event)
{
    event->accept();
}

void NeuronGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid Neuron address or malformed URI parameters."));
    }

    event->acceptProposedAction();
}

void NeuronGUI::handleURI(QString strURI)
{
...
void NeuronGUI::setEncryptionStatus(int status)
{
...
void NeuronGUI::encryptWallet(bool status)
{
    if(!walletModel)
        return;
...
void NeuronGUI::changePassphrase()
{
...
void NeuronGUI::showNormalIfMinimized(bool fToggleHidden)
{
...
void NeuronGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void NeuronGUI::updateWeight()
{
    if (!pwalletMain)
        return;
...
void NeuronGUI::updateStakingIcon()
{
