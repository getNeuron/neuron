...
QString TransactionRecord::getTypeLabel(const int &type)
{
    ...
    case RecvWithAddress:
        return NeuronGUI::tr("Received with");
    case RecvFromOther:
        return NeuronGUI::tr("Received from");
    case SendToAddress:
    case SendToOther:
        return NeuronGUI::tr("Sent to");
    case SendToSelf:
        return NeuronGUI::tr("Payment to yourself");
    case Generated:
        return NeuronGUI::tr("Mined");
    case RecvNeuron:
        return NeuronGUI::tr("Received neuron");
    case SendNeuron:
        return NeuronGUI::tr("Sent neuron");
    case Other:
        return NeuronGUI::tr("Other");
    default:
        return "";
    }
}
...
QString TransactionRecord::getTypeShort(const int &type)
{
    ...
    case TransactionRecord::RecvNeuron:
        return "input";
    case TransactionRecord::SendToAddress:
    case TransactionRecord::SendToOther:
    case TransactionRecord::SendNeuron:
        return "output";
    default:
        return "inout";
    }
...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet *wallet, const CWalletTx &wtx)
{
    ...
    int64_t nCredNRN, nCredNeuron;
    wtx.GetCredit(nCredNRN, nCredNeuron, true);
    int64_t nCredit = nCredNRN + nCredNeuron;
    ...
    {
        if (nNet > 0 && nCredNeuron > 0)
        {
            // -- credit
            TransactionRecord sub(hash, nTime, TransactionRecord::RecvNeuron, "", "", nNet, 0);
        ...
        {
            // -- debit
            TransactionRecord sub(hash, nTime, TransactionRecord::SendNeuron, "", "", nNet, 0);
        ...
        for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
        {
                ...
                && txout.IsAnonOutput())
            {
                ...
                if (wallet->HaveKey(ckidD))
                {
                    ...
                    sub.type = TransactionRecord::RecvNeuron;
    ...
    } else
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, wtx.vin)
        {
                ...
                && txin.IsAnonInput())
            {
                ...
                {
                    fAllFromMe = false;
                    break; // display as send/recv neuron
                };
                continue;
            };
        ...
        if (fAllFromMe)
        {
            ...
            for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
            {
                    ...
                    && txout.IsAnonOutput())
                {
                    ...
                    sub.type = TransactionRecord::SendNeuron;
                    sub.address = CBitcoinAddress(ckidD).ToString();
