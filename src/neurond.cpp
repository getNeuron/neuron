...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        {
            // First part of help message is specific to neurond / RPC client
            std::string strUsage = _("Neuron version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  neurond [options]                     " + "\n" +
                  "  neurond [options] <command> [params]  " + _("Send command to -server or neurond") + "\n" +
                  "  neurond [options] help                " + _("List commands") + "\n" +
                  "  neurond [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "neuron:"))
                fCommandLine = true;
...
int main(int argc, char* argv[])
{
    ...
    // Connect neurond signal handlers
    noui_connect();
